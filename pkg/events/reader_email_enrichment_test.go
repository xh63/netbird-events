...
// TestEmailEnrichment_AisOktaUsers tests email enrichment using okta_users table
func TestEmailEnrichment_AisOktaUsers(t *testing.T) {
	...
	reader := NewPostgresEventReader(db, logger, emailConfig)
	...
	// Expect query with okta_users JOIN
	mock.ExpectQuery("SELECT e.id, e.timestamp, e.activity, e.initiator_id, e.target_id, e.account_id, e.meta.*FROM events e.*LEFT JOIN okta_users u1.*LEFT JOIN okta_users u2").
		WillReturnRows(rows)
...
func TestEmailEnrichment_Auto(t *testing.T) {
	...
	mock.ExpectQuery("SELECT e.id, e.timestamp, e.activity, e.initiator_id, e.target_id, e.account_id, e.meta.*COALESCE.*okta.email.*netbird.email.*FROM events e.*LEFT JOIN okta_users okta.*LEFT JOIN users netbird").
		WillReturnRows(rows)
