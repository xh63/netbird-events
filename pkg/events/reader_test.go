...
func (m *mockEmailEnrichmentConfig) GetDecryptionKey() ([]byte, error) {
	return nil, nil
}
...
func TestNewPostgresEventReader(t *testing.T) {
	...
	pgReader, ok := reader.(*PostgresEventReader)
	if !ok {
		t.Fatal("Expected *PostgresEventReader")
	}
	if pgReader.db != db {
		t.Error("Expected db to be set correctly")
	}
	if pgReader.logger != logger {
